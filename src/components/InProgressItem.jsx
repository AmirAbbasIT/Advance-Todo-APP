...
import { Trash, Edit, CheckCircle, Tag } from 'react-feather';
...
function InProgressItem({ item, onDelete, onComplete, setAnimate, handleAddTag, deleteTag, onEdit }) {
        ...
        <>
            <ListItem className={`${item.animate ? "fade-in" : ""} ${state ? "fade-out" : ""}`}>
                ...
                <div>
                    ...
                    <Button onClick={onComplete}> <CheckCircle size={15} /></Button>
                    <Button onClick={() => onEdit(item.id, item.name)}> <Edit size={15} /></Button>
...
export default observer(InProgressItem)
